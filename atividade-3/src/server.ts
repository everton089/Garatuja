...
const server = Bun.serve({
  ...
  async fetch(request: Request) {
    ...
    // GET /items?index=0 - listar um item específico
    if (pathname === "/items" && method === "GET") { 
        const index = parseInt(searchParams.get("index") || "");
        const items = await todo.findItemByIndex(index);
        if (isNaN(index)) {
    ...
    // GET /items?description=foo - listar um item específico
    if (pathname === "/items" && method === "GET") {
        const description = searchParams.get("description") || "";
        const items = await todo.findItemByDescription(description);
        if (description) {
            return new Response(JSON.stringify(items), {
                headers: { "Content-Type": "application/json" }
            });
